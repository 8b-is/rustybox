...
fn main() {
  ...
  let (name, argv): (String, Vec<&str>) =
    if prog == "rustybox-core" || prog == "rustybox" || prog.is_empty() {
      ...
      (
        raw[1].clone(),
        raw[1..].iter().map(String::as_str).collect(),
      )
    } else {
...
fn dispatch(name: &str, argv: &[&str]) -> Option<i32> {
  match name {
    ...
    "basename" => Some(uu_basename::uumain(
      argv.iter().map(std::ffi::OsString::from),
    )),
    "dirname" => Some(uu_dirname::uumain(
      argv.iter().map(std::ffi::OsString::from),
    )),
    "readlink" => Some(uu_readlink::uumain(
      argv.iter().map(std::ffi::OsString::from),
    )),
    ...
    "realpath" => Some(uu_realpath::uumain(
      argv.iter().map(std::ffi::OsString::from),
    )),
    ...
    "timeout" => Some(uu_timeout::uumain(
      argv.iter().map(std::ffi::OsString::from),
    )),
    ...
    "truncate" => Some(uu_truncate::uumain(
      argv.iter().map(std::ffi::OsString::from),
    )),
    ...
    "unexpand" => Some(uu_unexpand::uumain(
      argv.iter().map(std::ffi::OsString::from),
    )),
    ...
    "sync" => {
      ...
      unsafe {
        let _ = std::ffi::CStr::from_ptr("sync".as_ptr() as *const _);
      } // hack to not pull nix here since rustybox-core is mostly pure
